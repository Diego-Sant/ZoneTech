...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
    ...
    const onDelete = async () => {
        try {
            setLoading(true);
            await axios.delete(`/api/${params.storeId}/marca/${data.id}`);
            
            router.refresh();
            toast.success("Marca excluída com sucesso!")
        } catch (error) {
            console.log(error)
            toast.error("Tenha certeza de remover todos os produtos dentro dessa marca primeiro!")
        } finally {
            setLoading(false);
            setOpen(false);
        }
        ...
        <>
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent>
                    ...
                    <DropdownMenuItem onClick={() => router.push(`/${params.storeId}/marca/${data.id}`)}>
